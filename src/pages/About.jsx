...
export default function About() {
  return (
    <main className="min-h-screen bg-ivory pt-24 pb-24">
      {/* Hero */}
      <section className="bg-white border-b border-stone/50 py-20 text-center">
        <div className="max-w-2xl mx-auto px-6">
          ...
          <p className="text-ink-400 mt-5 text-base leading-relaxed">
      ...
      {/* Story */}
      <section className="max-w-7xl mx-auto px-6 py-16 grid grid-cols-1 lg:grid-cols-2 gap-14 items-center">
        <div className="grid grid-cols-2 gap-4">
          ...
          <img src="/images/Creamish-coffe vanilla 2.png"         alt="Coffee candle"   className="w-full aspect-square object-cover rounded-2xl shadow-md mt-8" />
          <img src="/images/Pale yellow-citronella 2.png"         alt="Citronella"      className="w-full aspect-square object-cover rounded-2xl shadow-md -mt-4" />
          <img src="/images/White- lavender and eucalyptus 2.png" alt="Lavender 2"      className="w-full aspect-square object-cover rounded-2xl shadow-md mt-4" />
        </div>
        <div>
          <span className="section-label mb-4 block">Who We Are</span>
          <h2 className="font-display text-4xl text-ink-900 mb-6 leading-tight">
      ...
      {/* Values */}
      <section className="bg-offwhite border-y border-stone/50 py-20">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { icon: <Leaf size={24} />,  title: "Sustainability", desc: "Natural, eco-friendly waxes and recyclable packaging. The planet matters to us as much as the perfect scent." },
              { icon: <Heart size={24} />, title: "Craftsmanship",  desc: "Every candle is hand-poured with care. We never rush the process — quality is our non-negotiable." },
              { icon: <Users size={24} />, title: "Community",      desc: "We employ local artisans and source ingredients responsibly, giving back to the community that inspires us." },
            ].map((v) => (
              <div key={v.title} className="card p-8 text-center hover:-translate-y-1 transition-transform duration-300">
                <div className="text-gold-400 flex justify-center mb-4">{v.icon}</div>
                <h3 className="font-serif text-ink-900 text-lg font-semibold mb-3">{v.title}</h3>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5 text-center">
          {[
            ...
            <div key={s.label} className="card p-6">
              <p className="font-display text-4xl text-gold-400 mb-2">{s.number}</p>
